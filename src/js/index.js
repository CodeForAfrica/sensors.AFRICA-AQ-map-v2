...
let openedGraph1 = []; // Stores a list of sensor ids
...
const scale_options = {
  ...
  Pressure: {
    valueDomain: [
      926, 947.75, 969.5, 991.25, 1013, 1034.75, 1056.5, 1078.25, 1100,
    ],
    colorRange: [
      "#dd2e97",
      "#6b3b8f",
...
window.onload = function () {
  ...
  d3.select("#map-info").html(
    translate.tr(
      lang,
      "<p>Hexagons show the median sensor value in each area for the selected option (PM10, PM2.5, temperature, humidity, pressure, AQI). See the scale on the left.</p>\
<p>Click a hexagon to view sensor IDs and values. The first row shows the number of sensors and the median value.</p>\
<p>Click the plus next to a sensor ID to view its recent measurements. Data updates every 5 minutes.</p>\
<p>AQI is calculated per US EPA guidelines. More info: <a href='https://www.airnow.gov/index.cfm?action=aqibasics.aqi'>Link</a>. Hover over the AQI scale for health details.</p>"
    )
  );
  ...
  function retrieveData() {
    api
      .getData("https://api.sensors.africa/v2/nodes/list-nodes/?format=json", 1)
      .then(function (result) {
        ...
        api
          .getData(
            "https://api.sensors.africa/v2/nodes/list-nodes/?format=json",
            2
          )
          .then(function (result) {
...
function displayGraph(id) {
  let inner_pre = "";
  let panel_str = `<iframe src="https://grafana.aq.sensors.africa/d-solo/2aa4b9d0-1c40-448e-a32b-271790875fed/pm-data?orgId=1&timezone=browser&var-sensor_id=<SENSOR>&var-hours=24&panelId=<PANELID>&__feature.dashboardSceneSolo=true" width="450" height="200" frameborder="0"></iframe>`;
  const sens = id.substr(3); // extract "id_" from id
  ...
  if (!openedGraph1.includes(sens)) {
    ...
    let iframes = d3.selectAll("iframe");
  } else {
    if (user_selected_value !== "Official_AQI_US") inner_pre = "(+) ";
    d3.select("#id_" + sens).html(inner_pre + "#" + sens);
